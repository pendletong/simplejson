  ...
  describe("JSON Pointer tests", [
    describe("normal path", [
      ...
      it("test 3", fn() {
        ...
        |> expect.to_be_ok
        |> expect.to_equal(JsonString("bar"))
        Nil
      }),
      it("test 4", fn() {
        simplejson.apply_pointer(basic_json, "/")
      ...
      it("test 6", fn() {
        ...
        |> expect.to_be_ok
        |> expect.to_equal(JsonNumber(Some(2), None, Some("2")))
        Nil
      }),
      it("test 7", fn() {
        simplejson.apply_pointer(basic_json, "/e^f")
        |> expect.to_be_ok
        |> expect.to_equal(JsonNumber(Some(3), None, Some("3")))
        Nil
      }),
      it("test 8", fn() {
        simplejson.apply_pointer(basic_json, "/g|h")
        |> expect.to_be_ok
        |> expect.to_equal(JsonNumber(Some(4), None, Some("4")))
        Nil
      }),
      it("test 9", fn() {
        simplejson.apply_pointer(basic_json, "/i\\\\j")
        |> expect.to_be_ok
        |> expect.to_equal(JsonNumber(Some(5), None, Some("5")))
        Nil
      }),
      it("test 10", fn() {
        simplejson.apply_pointer(basic_json, "/k\\\"l")
        |> expect.to_be_ok
        |> expect.to_equal(JsonNumber(Some(6), None, Some("6")))
        Nil
      }),
      it("test 11", fn() {
        simplejson.apply_pointer(basic_json, "/ ")
        |> expect.to_be_ok
        |> expect.to_equal(JsonNumber(Some(7), None, Some("7")))
        Nil
      }),
      it("test 12", fn() {
