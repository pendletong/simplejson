...
import simplejson/jsonvalue.{
  type ParseError, InvalidCharacter, InvalidEscapeCharacter, InvalidGrapheme,
...
const failing_tests = [
  Test(
    ...
    Error(InvalidGrapheme("D800", "D800\\uD800\\n\"]", 4)),
    Error(InvalidGrapheme("D800", "D800\\uD800\\n\"]", 4)),
  ),
  Test(
  ...
  Test(
    ...
    Error(InvalidGrapheme("DD1E", "Dd1ea\"]", 4)),
    Error(InvalidGrapheme("DD1E", "Dd1ea\"]", 4)),
  ),
  Test(
...
pub fn simplejson_tests() {
  ...
  |> list.map(fn(name) {
    it(name, fn() {
      ...
      let parsed = simplejson.parse_with_metadata(json)
        ...
        Ok("i" <> _) | Ok("y" <> _) -> {
          case list.find(failing_tests, fn(e) { e.file == name }) {
            Ok(Test(_, erlang_err, js_err)) -> {
              ...
              let res = case res {
                Error(err) -> Error(err)
                Ok(str) -> simplejson.parse_with_metadata(str)
              }
